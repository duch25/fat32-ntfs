...
void FAT32::Read_RDET(int sector_index)
{
    string file_name = "";
    string type = "";
    ...
    for (int i = firstEntry; i < totalEntry; i++ ) 
    {
        ...
        if((FAT32::Get_Value_Little_Endian(RDET, i*32 , 1) == 0x00)  || (FAT32::Get_Value_Little_Endian(RDET, i * 32, 1) == 0xE5))
            continue;
        ...
        else if (FAT32::Get_Value_Little_Endian(RDET, i*32 + 0x0B, 1) == 0x10 || FAT32::Get_Value_Little_Endian(RDET, i*32 + 0x0B, 1) == 0x20)
        {
            ...
            if (FAT32::Get_Value_Little_Endian(RDET, i*32 + 0x0B, 1) == 0x10)  // Folder
            {
                ...
                Element element(file_name, (first_cluster - 2) * sectors_per_cluster + first_sector_of_data, 0, type);
                ...
                cout << endl;
                vector<Element> children = this->Read_SDET((first_cluster - 2) * this->sectors_per_cluster + this->first_sector_of_data);
                for (auto c : children) {
                    element.items.push_back(c);
                }
            ...
            else  if (FAT32::Get_Value_Little_Endian(RDET, i*32 + 0x0B, 1) == 0x20)
            {
                ...
                type = Get_String(RDET, i*32 + 8, 3);
                Element element(file_name, (first_cluster - 2) * sectors_per_cluster + first_sector_of_data, FAT32::Get_Value_Little_Endian(RDET, i*32 + 28, 4), type);
...
vector<Element> FAT32::Read_SDET(int sector_index)
{
    string file_name = "";
    string type = "";
...
void FAT32::Print_RDET()
{
    cout << "\n------------------------------------------------------------------------------------------------\n";
    cout << "\t \t\ \t \t \t \t RDET : " << endl;
    this->Read_RDET(this->sectors_of_bootsector + numbers_of_fats * sector_per_FAT);
}
...
void FAT32::Print_Directory_File_Tree()
{
    ...
    for (int i = 0; i < this->list_file.size(); i++) {
        this->list_file[i].getElement(); // element
    }
}
// Doc data cua sector
...
Element FAT32::find(Element e, int id) {
    if (e.getID() == id) {
        return e;
    }
    Element target;
...
void FAT32::Read_File_Content(int ID)
{
    int found = 0;
    for (int i = 0; i < this->list_file.size(); ++i) {
        Element target = find(this->list_file[i], ID);
        if (target.getID() == ID) {
            target.getElement(); // in ten
            found = 1;
            if (!target.isFolder) {
                string type = target.getType();
                if (type == "txt" || type == "TXT")
                {
                    int size = target.getSize(), index = ID;
